...
const defaultError =(message)=> ElMessage.error(message+"   出现错误!请联系管理员")
...
function  get(url,data,success,failure=defaultFailure,error=defaultError){
    axios.post(url,{
        ...
        const res = response.data;
        if(res.success)
            success(res.message,response.status)
        else
            failure(res.message || '请求失败',response.status)
    }).catch((err) => {
        // 关键：捕获所有异常并调用错误回调
        if (err.response) {
            failure(data?.message || `服务器错误: ${err.response.status}`, err.response.status);
        } else if (err.request) {
            // 请求已发送但无响应（如网络断开）
            error('网络错误，请检查连接',0);
        } else {
